...
import { BusIcon, ClockIcon, CalendarIcon, SunIcon, MoonIcon, RouteIcon, ResetIcon } from './components/Icons';

// Helper function to find active looping services
const findActiveLoopingServices = (loopingServices: LoopingInfo[] | undefined, now: Date): UpcomingDeparture[] => {
    ...
    for (const loop of loopingServices) {
        ...
        if (nowTotalMinutes >= loopStartMinutes && nowTotalMinutes <= loopEndMinutes) {
            activeLoops.push({
                time: '--',
                minutesUntil: 0,
                isTomorrow: false,
...
// Helper function updated to handle the new data structure
const findUpcomingDepartures = (departures: DepartureInfo[], now: Date, count: number): UpcomingDeparture[] => {
  if (count <= 0) return [];
  const nowTotalMinutes = now.getHours() * 60 + now.getMinutes();
  const upcoming: UpcomingDeparture[] = [];

  // Find departures for "today" relative to the query time
  for (const departure of departures) {
    const [hour, minute] = departure.time.split(':').map(Number);
    const departureTotalMinutes = hour * 60 + minute;
    if (departureTotalMinutes > nowTotalMinutes) {
      upcoming.push({
        ...departure,
        minutesUntil: departureTotalMinutes - nowTotalMinutes,
        isTomorrow: false
      });
    }
  ...
  // If not enough found for today, add from "tomorrow"
  const remainingCount = count - upcoming.length;
  if (remainingCount > 0) {
    const tomorrowDepartures = departures.slice(0, remainingCount);
    for (const departure of tomorrowDepartures) {
      const [hour, minute] = departure.time.split(':').map(Number);
      const departureTotalMinutes = hour * 60 + minute;
      const minutesUntil = (24 * 60 - nowTotalMinutes) + departureTotalMinutes;
      upcoming.push({
        ...departure,
...
const DepartureCard: React.FC<{ route: Route; departures: UpcomingDeparture[] }> = ({ route, departures }) => (
    ...
    <div className="space-y-4">
      {departures.length > 0 ? (
        ...
        <div className="text-center py-4 bg-slate-700/50 rounded-lg">
          <p className="text-slate-400">今日已无班次</p>
        </div>
      )}
    </div>
  </div>
            ...
            {routes.map((route, index) => {
                ...
                return (
                    <div key={index} className="bg-slate-800/50 rounded-lg p-4">
                        ...
                        <div className="space-y-4">
                            {Object.entries(departuresByRoute).map(([routeDetail, times]) => (
                                <div key={routeDetail}>
                                    <p className="text-sm text-slate-300 mb-2 flex items-center gap-2">
                                        <RouteIcon className="w-4 h-4 flex-shrink-0" />
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setScheduleType(day === 0 || day === 6 ? 'holiday' : 'weekday');
  }, []);
  ...
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newQueryTime = new Date(queryTime);
  ...
  const upcomingDepartures = useMemo(() => {
    ...
    return activeSchedule.map(route => {
        const activeLoops = findActiveLoopingServices(route.loopingServices, queryTime);
        const timedDepartures = findUpcomingDepartures(route.departures, queryTime, 2 - activeLoops.length);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 mb-8 shadow-2xl border border-slate-700">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center bg-slate-900 rounded-full p-1">
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors duration-300 flex items-center gap-2 ${scheduleType === 'weekday' ? 'bg-cyan-500 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors duration-300 flex items-center gap-2 ${scheduleType === 'holiday' ? 'bg-cyan-500 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
                    >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <label htmlFor="queryTime" className="text-sm font-medium text-slate-300">
                    查询时间：
                </label>
                <input
                    type="time"
        ...
        <main className="space-y-8">
            <h2 className="text-2xl font-bold text-slate-300 flex items-center gap-3">
                ...
                <span className="capitalize text-cyan-400">{scheduleType === 'weekday' ? '工作日' : '节假日'}</span> - 即将发车
            </h2>
          {upcomingDepartures.map((route, index) => (
            <DepartureCard key={index} route={route} departures={route.upcoming} />
          ))}
        </main>
